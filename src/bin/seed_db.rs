...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Seed Eras (Bengali)
    sqlx::query("INSERT INTO eras (name, description, start_date, end_date) VALUES 
    ...
    // Seed Events (Bengali)
    sqlx::query("INSERT INTO events (era_id, title, description, event_date) VALUES 
    ...
    let rev_id = sqlx::query_scalar::<_, i64>("SELECT id FROM events WHERE title = 'প্রথম ওহী'").fetch_one(&pool).await?;

    // Seed Questions (Bengali)
    sqlx::query("INSERT INTO questions (event_id, question_text, explanation) VALUES 
        (?, 'প্রথম ওহী কোথায় নাজিল হয়েছিল?', 'এটি জাবাল আল-নূরের হেরা গুহায় নাজিল হয়েছিল।')
    ")
    .bind(rev_id)
    .execute(&pool).await?;

    let q_id = sqlx::query_scalar::<_, i64>("SELECT id FROM questions WHERE question_text LIKE 'প্রথম ওহী%'").fetch_one(&pool).await?;
    ...
    sqlx::query("INSERT INTO options (question_id, option_text, is_correct) VALUES 
        ...
        (?, 'উহুদ পাহাড়ে', 0)
    ")
    .bind(q_id).bind(q_id).bind(q_id).bind(q_id)
    .execute(&pool).await?;
